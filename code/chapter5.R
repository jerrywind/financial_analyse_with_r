...
#### Figure 5.5
#AS# you can comment if installed ,如果已经安装可以注释掉
#install.packages("moments")
#AE# 
#>#
library(moments)
KurtYlogrets = length(Ylogrets)^(-1)*sd(Ylogrets)^(-4)*sum((Ylogrets - mean(Ylogrets))^4)
KurtYlogrets
#~[1] 3.393385
kurtosis(Ylogrets)
#~[1] 3.412056
#measure Kurtosis of mixture
KurtZlogrets = length(Zlogrets)^(-1)*sd(Zlogrets)^(-4)*sum((Zlogrets - mean(Zlogrets))^4)
KurtZlogrets
#~[1] 12.09176
kurtosis(Zlogrets)
#~[1] 12.15829
...
#AE#
library(Quandl)
S2<-1/rev(Quandl('ECB/EURCHF',start_date="2014-01-30",end_date="2015-01-29")[,2])
...
for(path in b:205) {
  ...
  Y <- tmixture(N,diffLogS351dailyVol,
                ...
                17.84*diffLogS351dailyVol)if(path == b)
